...
class Backyard extends FlxState // to be used as template
{
	...
	override public function create()
	{
		...
		if (FlxG.save.data.framerate == 15)
			openfl.Lib.application.window.title = "Bambi's Corn Adventure | Alpha 5 on " + lime.app.Application.current.window.displayMode.refreshRate + " FPS";
		else
			openfl.Lib.application.window.title = "Bambi's Corn Adventure | Alpha 5 on " + FlxG.drawFramerate + " FPS";
		game = new FlxCamera();
		hud = new FlxCamera();
        ...
        if (FlxG.random.bool(1))
		    FlxG.sound.playMusic("assets/music/backyardRARE.ogg");
        else
            FlxG.sound.playMusic("assets/music/backyard.ogg");
		bambi = new Marcello();
		background = new FlxSprite(0, 0).loadGraphic("assets/images/sky.png");
		...
		add(achievementext);
		bambi.y = 900;
		if (DaveHouse.entered)
			bambi.x = -479;
		else
			bambi.x = 2519; // and goes to 1430
		bambi.stopmovements = true;
		bambi.setOffsets(-480, 2520);
		...
		new FlxTimer().start(0.6, function(move:FlxTimer){
			if (ThirdBranch.entered)
			{
				bambi.forcemovement2 = true;
				if (!FlxG.save.data.achievements.contains("Great Lengths"))
				{
					FlxG.save.data.achievements.push("Great Lengths");
					achievement("Great Lengths", "Grandes Distancias");
				}
			}
			else 
	...
	override public function update(elapsed:Float)
	{
		...
		if (bambi.x <= 1590 && bambi.forcemovement2)
		{
        ...
        {
            zoom = 0.8;
		    if (bambi.x >= 430 && bambi.x <= 1600)
		    	bcamerapos.x = bambi.getGraphicMidpoint().x;
		    bcamerapos.y = bambi.getGraphicMidpoint().y - 180;
        }
		...
		if (bambi.x >= bambi.xoffse2 && !transitioning && !bambi.forcemovement)
		{
			statetransition(new maps.ThirdBranch(), true);
		}
		if (bambi.x <= bambi.xoffse && !transitioning && !bambi.forcemovement2)
		{
			statetransition(new maps.DaveHouse(), true);
		}
	}
    ...
    function addTheParts()
    {
        var a:FlxSprite;
        var b:FlxSprite;
        var c:FlxSprite;
        a = new FlxSprite(0, 0, "assets/images/backyard/1.png");
        b = new FlxSprite(0, 0, "assets/images/backyard/2.png");
        c = new FlxSprite(0, 0, "assets/images/backyard/3.png");
        a.setGraphicSize(Std.int(a.width * 1.25));
        b.setGraphicSize(Std.int(b.width * 1.25));
        c.setGraphicSize(Std.int(c.width * 1.25));
        a.antialiasing = true;
        b.antialiasing = true;
        c.antialiasing = true;
        add(c);
