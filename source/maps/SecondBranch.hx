...
class SecondBranch extends FlxState
{
	...
	//	shader = new FlxSprite(-100, -100).makeGraphic(Std.int(FlxG.width * 4), Std.int(FlxG.height * 4), 0xFF21102b);
		...
		new FlxTimer().start(1.25, function(move:FlxTimer){if (ThirdBranch.entered){bambi.forcemovement = true;sniperguy.forcemovement = true;ThirdBranch.entered = false;} else{bambi.forcemovement2 = true;sniperguy.forcemovement2 = true;}});
		transitioning = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if (bambi.animation.curAnim.name.contains("walk") && !bambi.stopmovements && !gotsniper)
		{
		...
		if (bambi.x >= -1220 && bambi.forcemovement)
		{
		...
		{
			statetransition(new maps.Branches(), true);
		}
