...
class Left extends FlxState // to be used as template
{
	...
	override public function create()
	{
		...
		hud.bgColor.alpha = 0;
		if (!Tutorial.musicplayin)
		{
			FlxG.sound.playMusic("assets/music/corn.ogg");
			Tutorial.musicplayin = true;
		}
		bambi = new Marcello();
		background = new FlxSprite(0, 0).loadGraphic("assets/images/wait what.png");
		...
		new FlxTimer().start(0.6, function(move:FlxTimer){if (LeftAgain.entered){LeftAgain.entered = false;bambi.forcemovement = true;}else bambi.forcemovement2 = true;});
		transitioning = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if (bambi.x >= -40 && bambi.forcemovement)
		{
